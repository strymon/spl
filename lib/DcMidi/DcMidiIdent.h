...
*-------------------------------------------------------------------------*/
#ifndef DcMidiIdent_h__
#define DcMidiIdent_h__
...
static DcMidiId_t kMidiIdArray[] =\
{
...
typedef QHash<int,QByteArray> DcMidiIdHash_t;

class DcMidiIdsHash : public DcMidiIdHash_t
{
...
public:
    ...
    DcMidiIdsHash()
    {
        int len = sizeof(kMidiIdArray)/sizeof(DcMidiId_t);
...
static DcMidiIdsHash MidiNameToIds;
...
struct DcMidiDevIdent
{
    ...
    DcMidiDevIdent(const DcMidiData &data);

    /*!
    	Initialize device identity from the given identify response data
    */
    void            fromIdentData( const DcMidiData & data );
...
typedef QList<DcMidiDevIdent> DcMidiIdentList_t;

#endif // DcMidiIdent_h__
